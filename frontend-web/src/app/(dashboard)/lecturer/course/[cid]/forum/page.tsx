...
export default function LecturerForumPage() {
  ...
  const handleCreate = async () => {
    ...
    await topicsApi.create(courseId, { title, description: desc, pinned });
    setTitle(""); setDesc(""); setPinned(false); setShowCreate(false);
    loadTopics();
  };

  const handleDelete = async (tid: string) => {
    if (!confirm("Delete this topic and all its posts?")) return;
    await topicsApi.delete(courseId, tid);
    setTopics(prev => prev.filter(t => t.id !== tid));
    if (activeTopic?.id === tid) setActiveTopic(null);
  };

  const handleTogglePin = async (tid: string) => {
    const res = await topicsApi.togglePin(courseId, tid);
    setTopics(prev => prev.map(t => t.id === tid ? { ...t, pinned: res.pinned } : t)
  ...
  const handleDeletePost = async (postId: string) => {
    ...
    setTopics(prev => prev.map(t => t.id === activeTopic.id ? { ...t, reply_count: Math.max(0, t.reply_count - 1) } : t));
  };
        ...
        <>
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold text-white flex items-center gap-2">
              <MessageSquare className="w-6 h-6 text-accent-purple" /> Discussion Forum
            </h1>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-xs text-dark-500 mt-1">Create a topic to start a discussion.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {topics.map(t => (
                <div key={t.id}
                  ...
                  <div className="flex items-start gap-3" onClick={() => openTopic(t)}>
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={e => { e.stopPropagation(); handleTogglePin(t.id); }}
                        className={clsx("p-1.5 rounded-lg transition-colors",
                          t.pinned ? "text-accent-amber hover:bg-accent-amber/10" : "text-dark-400 hover:text-white hover:bg-white/5"
        ...
        <div className="flex flex-col h-[calc(100vh-12rem)]">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 mb-4">
            {postsLoading ? (
              ...
              return (
                <div key={p.id} className={clsx("glass-card p-3 max-w-[85%]", isMe ? "ml-auto" : "")}>
                  <div className="flex items-center gap-2 mb-1">
