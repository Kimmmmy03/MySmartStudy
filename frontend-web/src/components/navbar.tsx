...
export default function Navbar({ onMobileMenuToggle }: { onMobileMenuToggle?: () => void }) {
  ...
  return (
    <nav className="h-14 lg:h-16 glass border-b border-white/5 dark:border-white/5 flex items-center justify-between px-3 lg:px-6 sticky top-0 z-40 navbar-bar">
      <div className="flex items-center gap-2 lg:gap-3">
        {/* Mobile hamburger */}
        {onMobileMenuToggle && (
          <button
            onClick={onMobileMenuToggle}
            className="lg:hidden w-9 h-9 rounded-xl flex items-center justify-center hover:bg-black/5 dark:hover:bg-white/10 transition-colors"
            aria-label="Open menu"
          >
            <Menu className="w-5 h-5 text-gray-600 dark:text-dark-200" />
          </button>
        )}
        <div className="flex items-center gap-2 lg:gap-3 px-2 lg:px-3 py-1.5 rounded-xl bg-white/5 border border-white/8 navbar-logo-box">
          <Image
            src="/logo.png"
            alt="MySmartStudy"
            width={28}
      ...
      <div className="flex items-center gap-1.5 lg:gap-2">
        ...
        <button
          onClick={toggleTheme}
          className="w-9 h-9 rounded-xl flex items-center justify-center hover:bg-black/5 dark:hover:bg-white/10 transition-colors homepage-toggle-btn"
...
<NotificationDropdown kind="messages" />
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className="flex items-center gap-1.5 lg:gap-2.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-xl px-2 lg:px-3 py-1.5 lg:py-2 transition-all duration-200"
          >
            <img
              src={avatarUrl}
              alt="avatar"
              className="w-7 h-7 lg:w-8 lg:h-8 rounded-full ring-2 ring-ipg-navy/20 dark:ring-ipg-navy/30"
              onError={(e) => {
          ...
          <AnimatePresence>
            {dropdownOpen && (
              <motion.div
                ...
                className="absolute right-0 top-full mt-2 w-52 glass-card py-1 z-50 dropdown-menu"
              >
                <div className="px-4 py-3 border-b border-white/5 mobile-menu-border">
                  ...
                  <span className="inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded-full text-[10px] font-medium bg-ipg-navy/10 text-ipg-sky">
