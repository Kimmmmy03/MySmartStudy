"""
CLP (Course Learning Plan) — Configuration & System Prompts

Ported from CLP standalone project, adapted for MySmartStudy integration.
Uses centralized Gemini client from app.ai_service.
"""
...
class Settings:
    TEMPLATE_PATH: str = os.getenv(
        "CLP_TEMPLATE_PATH",
        str(Path(__file__).resolve().parent.parent.parent.parent / "templates" / "clp_template.xlsx"),
    )
    SAMPLES_DIR: str = os.getenv(
        "CLP_SAMPLES_DIR",
        str(Path(__file__).resolve().parent.parent.parent.parent / "samples"),
    )
...
def load_sample_output() -> str:
    """Load the example week output from samples/clp_example_week1.txt."""
    sample_path = Path(settings.SAMPLES_DIR) / "clp_example_week1.txt"
