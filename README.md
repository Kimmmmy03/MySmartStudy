...
**▶ [https://mysmartstudy-web-393385396386.asia-southeast1.run.app/](https://mysmartstudy-web-393385396386.asia-southeast1.run.app/)**

The web platform is deployed and publicly accessible on **Google Cloud Run** (region `asia-southeast1`). The FastAPI backend runs as a companion Cloud Run service; data is persisted in Firebase Firestore.

> 📄 **Full project report:** the complete FYP thesis — every chapter, table, figure and diagram — is available as a Markdown conversion in **[README_THESIS.md](README_THESIS.md)** (figures in [`docs/thesis_media/`](docs/thesis_media/)).
...
This release rebuilds the SmartBuddy chat around **transparent source attribution** and tightens the mind-map editor UX. Highlights:

### SmartBuddy chat — three-tier sourcing
Every substantive answer now shows where the information came from, and the model is prevented from inventing references:

| Tier | When it fires | Shown as |
|------|---------------|----------|
| 🎓 **Course** | RAG hit on lecturer-indexed material with cosine score ≥ 0.40 | Green chip — *"Course — [PDF / Announcement / …]"* |
| 📰 **Online** | Course tier empty → OpenAlex peer-reviewed papers from the **last 6 years** (filter `from_publication_date:` + `type:journal-article\|book\|book-chapter\|review-article\|proceedings-article\|report`) | Blue chip — clickable to DOI / open-access PDF |
| 🤖 **General knowledge** | Neither tier matched → Gemini answers from training data with **no inline citations** (the model used to fabricate "Vygotsky 1978" — we strip `[Source N]` markers and `CITATIONS_JSON` from this path) | Amber chip — *"please verify before relying"* |

A clean topic phrase is extracted from the user's question before querying OpenAlex (the full wordy question text gave 0 hits). `external_lookup.verify_citation` also enforces the 6-year cutoff defensively. The 1978-Vygotsky-unverified failure mode is now impossible.

### SmartBuddy chat — formatting + new CTAs
- Renders **markdown** (`react-markdown` on web, `flutter_markdown` on mobile) — `**bold**`, bullets, `###` headings, code, links.
- Style prompt rewritten to match ChatGPT/Claude/Gemini conventions: natural prose default, bullets only for real lists, no filler openers/closers.
- **Em dashes and en dashes are banned in chat output** (both at prompt level and via a defensive `replace("—", ", ")` strip).
- Three CTA chips on every study question: **Generate flashcards / Make a summary / Build a quiz**. Click → calls `POST /api/ai/study-materials/generate-by-topic` with the right `evidence_tier`, persists to `generatedStudyMaterials`, then opens the AI Study Materials page.

### SmartBuddy chat — reachable everywhere
- Web: added a `[Guide | Chat]` tab strip to the AiCompanionWidget so the chat is available on every dashboard page, not just inside the mind-map editor.
- Mobile: tapping the brain FAB now opens a bottom-sheet chooser between **SmartBuddy Chat** (new) and **Daily Guide & Plan** (existing).

### AI Study Materials — provenance banner
Every material generated by topic now carries `evidenceTier`, `provenanceBanner` (markdown), and `citations[]` Firestore fields, and the viewer renders a coloured banner at the top:

- 🎓 *Generated from your lecturer's course notes.*
- ⚠️ *NOT from your course notes. Sourced from academic literature (last 6 years):* + citation list.
- ⚠️ *NOT from your course notes. AI general knowledge with no verifiable academic sources for this topic. Cross-check key facts before relying on them.*

### Mind-map editor — UX polish
- **Drag-to-swap shapes** — drag a shape from the palette onto an existing node and a *Swap shape?* popup appears at the cursor, preserving label/position/colour/edges.
- **Auto-title** — the map title bar follows the most-connected ("main topic") node's label until the user manually edits the title.
- **Floating edges** — connector endpoints now sit on each node's *visible contour* (analytic intersection for circle/ellipse/diamond, rectangle fallback for other shapes) and recompute live on drag.
- **Shape follows recommended map type** — the SmartBuddy wizard now chooses an oval for spider maps, a circle for bubble maps, etc.
- **Per-node colour cycling** — palette of 14 colours rotated by node count so successive nodes are visually distinct (edge inherits the node's stroke colour).
- **Sequential build-out wizard** — clicking a node walks `Subtopic → Detail → Example → Image → Resource`. Detail-level recommendations now contextualise on (and attach under) the newly added subtopic instead of all hanging off the main topic.
- **NodeResizer fix** — every shape (not just rectangles) now resizes correctly.

### Backend changes
- New `backend/app/services/external_lookup.py` — OpenAlex client (no API key, 6-year recency filter, 1-hour in-process LRU, polite-pool email).
- Cloud Run memory bumped to **2 GiB** + `RAG_RERANK_ENABLED=0` env to avoid OOM-kills from the 2.2 GB cross-encoder reranker on the previous 1 GiB tier.

### Mobile parity (Flutter)
- New screen: `lib/screens/smart_buddy_chat_screen.dart` with markdown rendering + tier chips + three CTAs.
- New widget: `lib/widgets/provenance_banner.dart` used by summary / flashcard / quiz viewers.
- New models: `lib/models/ai_chat.dart` (`ChatResponse`, `ChatSource`, `ChatSuggestedAction`, `EvidenceTier`).
- `pubspec.yaml` adds `flutter_markdown: ^0.7.3` (alongside the existing `url_launcher`).
- Production APK: `frontend-mobile/build/app/outputs/flutter-apk/app-release.apk` (~103 MB).

> Trace these changes through the diagrams: **`8_ai_features_interaction.drawio`** and **`diagrams_simple/6_ai_features_interaction_simple.drawio`** are the canonical visualisations of the new chat tier pipeline.

## Project Context (Academic Overview)

**Project Title:** MySmartStudy — *Digital Platform for Enhancing Learning Skills among First Year Students, IPG Kampus Perempuan Melayu Melaka* (An AI-Enhanced Collaborative Learning Management System with Mind Map Integration)
...
**Authors:** Akmal Hakimi bin Abd Rashid · Mohamed Azeem bin Sulaiman
**Supervisor:** Sir Azrai bin Abdul Aziz **·** **Assessor:** Ts. Suguneswari Raja Gopal
...
**Date:** April 2026
**Live Deployment:** [https://mysmartstudy-web-393385396386.asia-southeast1.run.app/](https://mysmartstudy-web-393385396386.asia-southeast1.run.app/)
**Full Report:** [README_THESIS.md](README_THESIS.md) — complete thesis (all chapters, tables, figures & diagrams)
...
### AI Companion FAB (floating)

The brain-shaped FAB at the bottom-right of every dashboard screen now opens a **bottom-sheet chooser** instead of jumping straight into the Daily Guide:

| Option | Screen | What it is |
|--------|--------|-----------|
| **SmartBuddy Chat** *(new May 2026)* | `SmartBuddyChatScreen` | Full chat with markdown rendering (`flutter_markdown`), three-tier source chips (Course / OpenAlex / AI), and **Generate Flashcards / Make a Summary / Build a Quiz** CTA chips under every answer that hit `aiGenerateStudyMaterialByTopic` and route the user to the AI Study Materials screen. |
| **Daily Guide & Plan** | `AiCompanionScreen` | Learning-style assessment, daily AI guide, exam planner, timetable upload. |

Long-press still dismisses the orbiting brain bubbles (persisted via SharedPreferences).

### Provenance Banner

The three AI study material viewers (`AiSummaryViewer`, `AiFlashcardViewer`, `AiPracticeQuizScreen`) accept an optional `material` map and render a tier-coloured `ProvenanceBanner` widget at the top:

- 🎓 green — *Generated from your lecturer's course notes.*
- 📰 blue — *NOT from your course notes. Sourced from academic literature (last 6 years).* Each citation row is tappable and opens the DOI / open-access PDF via `url_launcher`.
- 🤖 amber — *NOT from your course notes. AI general knowledge with no verifiable academic sources.*
...
| **AI SmartBuddy Chat** *(new May 2026)* | `aiMindmapBuddyChat` | Three-tier sourced chat (course RAG / OpenAlex / general knowledge). Returns `ChatResponse` with `sources[]` and `suggested_actions[]`. Consumed by `SmartBuddyChatScreen`. |
...
| AI Materials | `aiGenerateStudyMaterial`, `aiGenerateStudyMaterialByTopic` *(new May 2026: accepts `evidenceTier`)*, `aiGetStudyMaterials`, `aiDeleteStudyMaterial` | AI-generated content. The topic-based generator powers the three CTA chips in SmartBuddy chat. |
...
- **Generate**:
  ...
  - `recommend-nodes` / `suggest-all` → GAG graph artifact: suggested nodes (with `source` citation back to a RAG chunk and `graph_connections[]` to existing nodes). The `recommend-nodes` call now takes a `rec_type` (`subtopic | detail | example | image | resource`) so the sequential build-out wizard can ask for one tier at a time.
  - `chat` → **three-tier sourced response** (see "What's New" above). Pipeline order:
    1. Course RAG (chunks with cosine score ≥ 0.40 form the `course` tier).
    2. If weak/empty AND the message is a study question (≥30 chars + a learning verb), a clean topic phrase is extracted and `external_lookup.lookup_openalex` is called with `from_publication_date` set to today − 6 years and `type` restricted to journal/book/book-chapter/review/proceedings/report. Hits form the `online` tier.
    3. If both empty, Gemini answers from general knowledge **without inline citations** (the model is explicitly told not to emit `[Source N]` markers or Author/Year refs — defensive `re.sub` strip on the response as belt-and-braces).
    Returns `{ response, evidence_level, sources[], suggested_actions[] }`. `suggested_actions` carries three entries (`flashcards`, `summary`, `quiz`) each with the extracted `topic` and the chosen `evidence_tier` — the frontend renders them as CTA chips that hit `generate-by-topic`.
- **Cache**: `aiMapAnalysisCache` 2h (whole-map analysis), `aiNodeRecsCache` 24h (per-node recs), `aiSuggestAllCache` (entire-map suggestions). OpenAlex lookups carry their own 1-hour in-process LRU keyed by `(query, top_k, recency_years)`.
...
| 2 | Study Materials | `POST /ai/study-materials/generate{,-by-topic}` | Standard RAG (course tier) / OpenAlex (online tier) / pure Gemini (general_knowledge tier) | FAST | resource(s), topic, `evidence_tier` (`course \| online \| general_knowledge`) | Markdown summary / flashcards / quiz JSON + tier-specific `provenanceBanner` + `citations[]` persisted on the material | 7d dedup keyed on `(user, tier, topic_hash, type)` |
...
**Live (published) site:** [https://mysmartstudy-web-393385396386.asia-southeast1.run.app/](https://mysmartstudy-web-393385396386.asia-southeast1.run.app/)

**Full FYP report (Markdown conversion of the thesis):** [README_THESIS.md](README_THESIS.md)
